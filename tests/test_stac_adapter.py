...
def test_stac_load_selects_only_the_explicit_asset_and_preserves_item() -> None:
    ...
    source = adapter.load(Config("stac", {"endpoint": endpoint, "collection_id": "sentinel-2", "item_id": "scene-1", "asset_key": "visual"}))
...
def test_stac_search_maps_spatial_temporal_and_collection_conditions() -> None:
    ...
    client = RecordingJsonClient({search_url: fixture_json("stac/item_collection.json")})
    adapter = StacAdapter(endpoint=endpoint, get_json=client)
    results = adapter.search(SearchQuery(bbox=(139.0, 35.0, 140.0, 36.0), limit=2), collections=("sentinel-2",))
    assert client.calls == [(search_url, {"bbox": "139.0,35.0,140.0,36.0", "collections": "sentinel-2", "limit": 2})]
    assert results[0].to_config() == Config("stac", {"collection_id": "sentinel-2", "item_id": "scene-1", "asset_key": "visual"})
...
def test_stac_requires_explicit_asset_key() -> None:
    with pytest.raises(ConfigValidationError, match="asset_key"):
        StacAdapter(get_json=RecordingJsonClient({})).load(Config("stac", {"endpoint": "https://stac.example", "collection_id": "sentinel-2", "item_id": "scene-1"}))
...
def test_stac_missing_requested_asset_is_a_response_error() -> None:
    ...
    with pytest.raises(ProviderResponseError, match="missing"):
        StacAdapter(get_json=client).load(Config("stac", {"endpoint": endpoint, "collection_id": "sentinel-2", "item_id": "scene-1", "asset_key": "missing"}))
