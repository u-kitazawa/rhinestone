...
def test_ogc_collection_items_link_becomes_service_resource() -> None:
    ...
    source = adapter.load(Config("ogc-features", {"endpoint": endpoint, "collection_id": "rivers"}))
    assert client.calls == [(collection_url, {})]
    assert source.metadata.title == "Rivers"
    assert source.candidates[0].uri == "https://features.example/collections/rivers/items"
    ...
    assert source.raw_metadata["extent"]["spatial"]["bbox"][0] == (139.0, 35.0, 140.0, 36.0)
...
def test_ogc_search_uses_items_endpoint_and_standard_query_parameters() -> None:
    ...
    client = RecordingJsonClient({items_url: fixture_json("ogc/feature_collection.json")})
    adapter = OgcFeaturesAdapter(endpoint="https://features.example", collection_id="rivers", get_json=client)
    ...
    assert results[0].to_config() == Config("ogc-features", {"collection_id": "rivers", "feature_id": "river-1"})
...
def test_ogc_search_requires_collection_context() -> None:
    adapter = OgcFeaturesAdapter(endpoint="https://features.example", get_json=RecordingJsonClient({}))
...
def test_ogc_collection_without_items_link_is_rejected() -> None:
    endpoint = "https://features.example"
    collection_url = endpoint + "/collections/rivers"
    client = RecordingJsonClient({collection_url: {"id": "rivers", "title": "Rivers", "links": [{"rel": "self", "href": collection_url}]}})
    with pytest.raises(ProviderResponseError, match="items"):
        OgcFeaturesAdapter(get_json=client).load(Config("ogc-features", {"endpoint": endpoint, "collection_id": "rivers"}))
