...
def test_estat_credential_factory_is_lazy_and_used_for_requests() -> None:
    ...
    client = RecordingJsonClient({search_url: fixture_json("estat/get_stats_list.json")})
    calls = []
...
def test_estat_metadata_response_becomes_service_source_without_leaking_app_id() -> None:
    ...
    client = RecordingJsonClient({metadata_url: fixture_json("estat/get_meta_info.json")})
    adapter = EStatAdapter(app_id="secret-app-id", get_json=client)
    source = adapter.load(Config("estat", {"stats_data_id": "0000000001"}))
    assert client.calls == [(metadata_url, {"appId": "secret-app-id", "statsDataId": "0000000001", "lang": "J"})]
    ...
    assert source.raw_metadata["METADATA_INF"]["CLASS_INF"]["CLASS_OBJ"][0]["@id"] == "area"
...
def test_estat_search_maps_text_and_limit_to_official_parameter_names() -> None:
    ...
    assert client.calls == [(search_url, {"appId": "secret-app-id", "searchWord": "人口", "limit": 10, "lang": "J"})]
...
def test_estat_nonzero_result_status_is_rejected() -> None:
    endpoint = "https://api.e-stat.go.jp/rest/3.0/app/json"
    metadata_url = endpoint + "/getMetaInfo"
    client = RecordingJsonClient({metadata_url: {"GET_META_INFO": {"RESULT": {"STATUS": 100, "ERROR_MSG": "認証に失敗しました。"}}}})
    with pytest.raises(ProviderResponseError, match="認証"):
        EStatAdapter(app_id="invalid", get_json=client).load(Config("estat", {"stats_data_id": "0000000001"}))
