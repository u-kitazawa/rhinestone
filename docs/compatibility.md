...
| `pyogrio` | representation registry で `vector` と明示された `shapefile`、`geojson`、`gpkg`、`flatgeobuf`、`gml`、`kml`、`citygml` | URI と encoding だけを `read_dataframe()` へ渡す。URI/ archive内容から形式を推測せず、環境に対応 driver がない場合は `ResourceAccessError`。archive URI の組み立てはしない。 |
