...
を参照してください。GDAL等の外部Runtimeは `dependencies` に実体またはfactoryとして渡します。JSON serviceのHTTP runtimeはRhinestoneが組み込みで提供します。依存境界とSourceごとの採用方針は[外部ライブラリ依存方針](../dependency-policy.md)を参照してください。
