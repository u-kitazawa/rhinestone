...
| V05-21 | 外部自作Adapterの登録・公開SDK契約 | `Implemented` | `configure(adapters=...)`、公開 Definition / Context / Credential・Dependency・Knowledge・Transport Port、重複 fail-fast、[Custom Adapterを作る](custom-adapters.md)、`tests/test_source_composition.py`、`tests/test_open_issue_contracts.py` | #85 で完了。自動 discovery 等は具体的な利用価値が確認された場合に別 Issue で再評価する |
...
- #85 は外部自作Adapterの公開 SDK closure を担当し、Definition / Context / Port と authoring guide / contract tests が揃ったため完了済みです。
- #136 は #49 の再評価結果を受け、複数 Source / Execution Adapter に重複している canonical Representation vocabulary の集約を担当します。
- V05-09bは既知の欠落として `Deferred` に固定します。実装時は両Sourceの記録をどう公開するかを専用Issueで決定し、既存の単一Source契約と分離して追跡します。
- release前のnetwork/credential security保証はrelease gateで追跡し、V05-15の責務分離モデルおよび #47 のarchitecture completionとは区別します。

## #47 の統合判定

2026-09-13 時点で、Knowledge Layer、e-Stat GIS、外部 Adapter SDK の主要ゲートは完了しています。現在の architecture integration 上の未完了ゲートは **#136 の Representation vocabulary 共通化**です。

1. V05-08: #49 の再評価条件が成立したため、#136 で canonical format / media type / archive semantics の producer-consumer vocabulary を Source / Resource / Execution 間で一貫させる。
2. V05-09b は明示的な `Deferred` であり、再評価条件が成立するまで #47 の blocker にしない。
3. V05-15 の release向け security hardening は release gate として分離し、#47 の blocker にしない。
4. V05-06、V05-13、V05-19 は再評価条件が成立するまで `Deferred` として扱い、暗黙の実装残件に戻さない。
5. V05-07、V05-16、V05-21 は完了済みであり、旧 Issue 状態を未完了ゲートとして扱わない。
